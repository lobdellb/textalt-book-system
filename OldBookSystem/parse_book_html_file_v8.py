...
if len( records ) == 0:
   records = tbody[0].contents[1].findChildren('tr',recursive=False)
...
for record_i in range(1,len(records),4):
   ...
   if len( info[10].findAll('span')[1].contents ) == 1:
      newpr_str = info[10].findAll('span')[1].contents[0].strip()
   else:
      newpr_str = ''

   #print len( info[13].findAll('span') )
   if len( info[13].findAll('span') ) > 1:
