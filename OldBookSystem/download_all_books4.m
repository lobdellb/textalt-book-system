...
% Request URI: /webapp/wcs/stores/servlet/TBListView
...
%         mn = magic_number(1:end-1);
         ...
         if magic_number(end) ~= 'Y' & good == 1
            ...
            %post_data = sprintf('storeId=36052&langId=-1&catalogId=10001&savedListAdded=true&clearAll=&viewName=TBWizardView&removeSectionId=&section_1=%s&sectionList=%s&viewTextbooks.x=&viewTextbooks.y=&sectionList=newSectionNumber',mn,mn);
                               % storeId=36052&langId=-1&catalogId=10001&savedListAdded=true&clearAll=&viewName=TBWizardView&removeSectionId=&mcEnabled=N&section_1=38078372&numberOfCourseAlready=1&viewTextbooks.x=36&viewTextbooks.y=6&sectionList=newSectionNumber           
            post_data = sprintf('storeId=36052&langId=-1&catalogId=10001&savedListAdded=false&clearAll=&viewName=TBWizardView&removeSectionId=&mcEnabled=N&section_1=%s&numberOfCourseAlready=1&viewTextbooks.x=36&viewTextbooks.y=6&sectionList=newSectionNumber',mn);
...
%                  disp(length(html))
                  ...
                  if length( findstr( html, 'Currently no textbook has been assigned for this course.' ) ) > 0
                     ...
                     delete('output.txt')
                     system('python parse_book_html_file_v8.py');
                     ...
                     while ~feof(fp)
                        ...
                        books(book_i).title = fgetl(fp);
                        books(book_i).author = fgetl(fp);
                        books(book_i).edition = fgetl(fp);
                        books(book_i).publisher = fgetl(fp);

                        isbn_url = fgetl(fp);     

                        if ~strcmp( isbn_url,'none' )
                            ...
                            while ~success & try_count < 10
                            
                               try
                                  try_count = try_count + 1;
                                  success = 1;
                                  system(sprintf('wget --output-document=temp.jpg http://iupui.bncollege.com%s >/dev/null',isbn_url));
                                  books(book_i).isbn = rec2( 'temp.jpg' );
                               catch
                                  success = 0;
                               end
                            end
                        ...
                        books(book_i).required = fgetl(fp);

                        books(book_i).no_book = 'book_listed';
                        
                        newpr_str = fgetl(fp);
                        usedpr_str = fgetl(fp);

                        books(book_i).new_price = round( sscanf(newpr_str(2:end),'%f') * 100 );
                        books(book_i).used_price = round( sscanf(usedpr_str(2:end),'%f') * 100 );

                        books(book_i).productid = fgetl(fp);


%                          disp(books(book_i).title)
%                          disp(books(book_i).author)
%                          disp(books(book_i).edition)
%                          disp(books(book_i).publisher)
%                          disp(books(book_i).isbn) 
%                          disp(books(book_i).required)
%                          disp(sprintf('%i',books(book_i).new_price))
%                          disp(sprintf('%i',books(book_i).used_price))
%                          disp(books(book_i).productid) 
...
%              end
            ...
            dept(dept_index).classes(m + class_index).section_books(p).books(1).author = [];
            dept(dept_index).classes(m + class_index).section_books(p).books(1).edition = [];
            dept(dept_index).classes(m + class_index).section_books(p).books(1).new_price = [];
            dept(dept_index).classes(m + class_index).section_books(p).books(1).no_book = 'none_req';
            dept(dept_index).classes(m + class_index).section_books(p).books(1).publisher = [];
            dept(dept_index).classes(m + class_index).section_books(p).books(1).required = [];
            dept(dept_index).classes(m + class_index).section_books(p).books(1).title = [];
            dept(dept_index).classes(m + class_index).section_books(p).books(1).used_price = [];
            dept(dept_index).classes(m + class_index).section_books(p).books(1).isbn = [];
