...
for n = 1:length(dept_strs)


   % need to check and see if this is a duplicate department 
   dept_str = char( dept_strs(n) );

   course_prefix = tmp(I+1:end);

   % find it in the existing list
   found = 0;
   for idx = 1:length(dept)
      if strcmp( char(dept(idx).str), dept_str )
   ...
   if length( I ) > 0
      class_prefix = [ dept_str(I+1:end) ' '];
      dept_str = dept_str(1:I-1);
   else
      class_prefix = [];
   end
   ...
   for m = 1:length(course_strs)
      ...
      dept(dept_index).classes(class_index+m).str = {[ course_prefix char(course_strs(m))]};
      dept(dept_index).classes(class_index+m).magic_number = course_magic_number(m);
      ...
      if exist([temp_dir 'section_list.html'],'file')
      ...
      dept(n).classes(class_index+m).section_strs = section_strs;
      dept(n).classes(class_index+m).section_magic_numbers = section_magic_number;
